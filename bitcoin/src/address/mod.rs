...
impl<V: NetworkValidation> Address<V> {
    ...
    Network::Testnet => PUBKEY_ADDRESS_PREFIX_TEST,
...
let hrp = match self.network() {
    Network::Bitcoin => hrp::BC,   // mainnet HRP constant provided by bech32
    Network::Testnet => hrp::TB,   // testnet HRP
    Network::Regtest => hrp::BCRT, // regtest HRP
};
...
if is_legacy {
    ...
    AddressType::P2pkh => {
        network == *self.network()
            || (*self.network() == Network::Testnet && network == Network::Regtest)
            || (*self.network() == Network::Regtest && network == Network::Testnet)
    }
    AddressType::P2sh => {
        network == *self.network()
            || (*self.network() == Network::Testnet && network == Network::Regtest)
...
fn test_p2pkh_address_58_v0() {
    // Build the address from the known hash + network (no hardcoded Base58 literal).
    ...
    // Roundtrip: parse from the string we produce and ensure equality.
    ...
    fn test_p2pkh_address_58() {
        ...
        assert_eq!(&addr.to_string(), "B6UKdoJfdRmYFyo9wpWFWDab19XgUvoL1P");
        assert_eq!(addr.address_type(), Some(AddressType::P2pkh));
        roundtrips(&addr);
    }
    ...
    fn test_p2pkh_from_key() {
        ...
        assert_eq!(&addr.to_string(), "BTkZqi63LX5j33oHWhywcfmpW2TgvsjBpd");
    ...
    fn test_address_debug() {
        ...
        let addr_str = "B6UKdoJfdRmYFyo9wpWFWDab19XgUvoL1P";
    ...
    fn test_address_type() {
        let addresses = [
            ("BTkZqi63LX5j33oHWhywcfmpW2TgvsjBpd", Some(AddressType::P2pkh)),
    ...
    fn test_json_serialize() {
        ...
        let addr =
            Address::from_str("B6UKdoJfdRmYFyo9wpWFWDab19XgUvoL1P").unwrap().assume_checked();
        let json = serde_json::to_value(&addr).unwrap();
        assert_eq!(
            json,
            serde_json::Value::String("B6UKdoJfdRmYFyo9wpWFWDab19XgUvoL1P".to_owned())
        );
    ...
    fn test_qr_string() {
        for el in
            ["B6UKdoJfdRmYFyo9wpWFWDab19XgUvoL1P", "9tTWgUQoVtNuogNtsZWJ3qmE7dkrPTrVAj"].iter()
        {
            let addr =
...
const SEGWIT_EQUIVALENCE_CLASSES: &[&[Network]] =
    &[&[Network::Bitcoin], &[Network::Regtest], &[Network::Testnet]];
    ...
    fn test_is_related_to_pubkey_p2shwpkh() {
        let pubkey_string = "0347ff3dacd07a1f43805ec6808e801505a6e18245178609972a68afbc2777ff2b";
        let pubkey = PublicKey::from_str(pubkey_string).expect("pubkey");
        // Build the address from the same pubkey so the relation must hold.
        let address = Address::p2shwpkh(&pubkey, Network::Bitcoin).unwrap();
    ...
    fn test_is_related_to_pubkey_p2pkh() {
        let address_string = "BMWR7JEw2sUwkb1m7LufW3CsBse1GxA2dP";
        let address = Address::from_str(address_string)
            .expect("address")
            .require_network(Network::Bitcoin)
            .expect("mainnet");
    ...
    fn test_is_related_to_pubkey_p2pkh_uncompressed_key() {
        ...
        let pubkey = PublicKey::from_str(pubkey_string).expect("pubkey");
        // Uncompressed is fine for legacy P2PKH – derive the address from this key.
    ...
    fn test_matches_script_pubkey() {
        let addresses = [
            "BTkZqi63LX5j33oHWhywcfmpW2TgvsjBpd",
            "BMWR7JEw2sUwkb1m7LufW3CsBse1GxA2dP",
            "9tTWgUQoVtNuogNtsZWJ3qmE7dkrPTrVAj",
            "AEvgWMSjuF9fYX2KXo2AWyWwbPSiueVmvj",
